...
public interface BLS12381 {
    ...
    Signature popProve(SecretKey sk);

    boolean popVerify(PublicKey sk, Signature proof);

    Signature signAugmented(SecretKey sk, byte[] message);

    boolean aggregateVerifyAugmented(List<? extends PublicKey> pks, List<byte[]> messages, Signature signature);
}
